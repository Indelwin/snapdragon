...
export function formatInspectWorker(worker: GatewayWorkerRecord): string {
  const suffixes = [
    ...
    listSuffix(' caps=', worker.capabilities),
...
export function formatInspectWorkerProcess(process: GatewayWorkerProcess): string {
  const suffixes = [numberSuffix(' pid=', process.pid), suffix(' error=', process.lastError)];
  return `${process.id}\t${process.service}\t${process.state}${suffixes.join('')}`;
}
